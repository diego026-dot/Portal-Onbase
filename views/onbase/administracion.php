    ...
    <div class="pd-ltr-20 xs-pd-20-10">
		<div class="min-height-200px">
            <div class="page-header mb-5">
					...
					<?php foreach($this->consultaIndicadores as $row){ //INICIO DEL FOR?>  
...
<?php } ?>
  ...
  <tbody>
  <?php
    foreach($this->consultaIndicadores as $row){ //INICIO DEL FOR
  ?>
    <tr>
      <td>
	  <button type="button" class="btn-sm btn btn-link"  
                    onclick="modal_usuario('<?php echo rtrim(ltrim($row->Usuario));?>','<?php echo rtrim(ltrim($row->Usuario));?>');" data-toggle="modal"
					 data-target="#AgregarPerm"<?php echo $row->Usuario;?>>
					<?php echo $row->Usuario;?>
                    </button>
				</td>
      <td><?php echo $row->Modulo;?></td>
	  <td><?php echo $row->Opcion;?></td>
	  <td width="10px"><button type="button" class="btn btn-danger btn-sm" onclick="elimina_permisos('<?php echo rtrim(ltrim($row->Usuario));?>');"
	  data-toggle="modal" data-target="#EliminarPerm"<?php echo $row->Usuario;?>>Eliminar Permisos </button></td>
    </tr>
	<?php } //FIN DEL FOR?>
  </tbody>  
</table>
...
function enviaDatos(){ 
		...
		if(opcion == '' || opcion == null) {opcion = 0;}
